...
AWS_REGION=$AWS_REGION helm repo add stable-petclinic s3://petclinic-helm-charts-catclinic/stable/myapp/ || echo "repository name already exists"
...
envsubst < k8s/petclinic_chart/values-template.yaml > k8s/petclinic_chart/values.yaml
sed -i s/HELM_VERSION/${BUILD_NUMBER}/ k8s/petclinic_chart/Chart.yaml
kubectl delete secret regcred -n petclinic-prod-ns || true
sh """
...
sh "helm repo add stable-petclinic s3://petclinic-helm-charts-<put-your-name>/stable/myapp/ || true"
